...
function inlineGoogleGenaiPlugin(): esbuild.Plugin {
	return {
		name: 'inline-google-genai',
		setup(build) {
			build.onResolve({ filter: /^@google\/genai$/ }, () => ({
				path: path.join(REPO_ROOT, 'node_modules/@google/genai/dist/web/index.mjs'),
				external: false,
			}));
			build.onResolve({ filter: /^p-retry$/ }, () => ({
				path: path.join(REPO_ROOT, 'node_modules/p-retry/index.js'),
				external: false,
			}));
			build.onResolve({ filter: /^retry$/ }, () => ({
				path: path.join(REPO_ROOT, 'node_modules/retry/index.js'),
				external: false,
			}));
		},
	};
 ...
 *--------------------------------------------------------*/`,
	...
	for (const entry of bootstrapEntryPoints) {
		...
		const bootstrapPlugins: esbuild.Plugin[] = [inlineMinimistPlugin(), inlineGoogleGenaiPlugin(), contentMapperPlugin];
		if (doNls) {
