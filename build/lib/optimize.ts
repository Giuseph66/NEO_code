			...
			const externalOverride: esbuild.Plugin = {
				...
				setup(build) {
					...
					build.onResolve({ filter: /^@google\/genai$/ }, () => {
						return { path: path.join(REPO_ROOT_PATH, 'node_modules', '@google', 'genai', 'dist', 'web', 'index.mjs'), external: false };
					});
					build.onResolve({ filter: /^p-retry$/ }, () => {
						return { path: path.join(REPO_ROOT_PATH, 'node_modules', 'p-retry', 'index.js'), external: false };
					});
					build.onResolve({ filter: /^retry$/ }, () => {
						return { path: path.join(REPO_ROOT_PATH, 'node_modules', 'retry', 'index.js'), external: false };
					});
				},
			};
