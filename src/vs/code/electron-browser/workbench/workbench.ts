...
(async function () {
	...
	function setupImportMaps<T extends ISandboxConfiguration>(configuration: T, baseUrl: URL) {
		...
		// Add @google/genai and its dependencies to import map for development mode
		const genaiUrl = new URL('../node_modules/@google/genai/dist/web/index.mjs', baseUrl).href;
		importMap.imports['@google/genai'] = genaiUrl;

		const pRetryUrl = new URL('../node_modules/p-retry/index.js', baseUrl).href;
		importMap.imports['p-retry'] = pRetryUrl;

		const retryUrl = new URL('../node_modules/retry/index.js', baseUrl).href;
		importMap.imports['retry'] = retryUrl;
