...
export class SearchView extends ViewPane {
	...
	private isFileNameSearchEnabled!: IContextKey<boolean>;
	...
	private fileNameResultsContainer!: HTMLElement;
	private textMatchesHeader!: HTMLElement;
	...
	constructor(
		...
		this.isFileNameSearchEnabled = Constants.SearchContext.FileNameSearchEnabledKey.bindTo(this.contextKeyService);
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		this.inputPatternIncludes = this._register(this.instantiationService.createInstance(IncludePatternInputWidget, folderIncludesList, this.contextViewService, {
			...
			placeholder: nls.localize('placeholder.includes', "ex: src/**, **/*.ts (glob patterns)"),
		...
		this.inputPatternExcludes = this._register(this.instantiationService.createInstance(ExcludePatternInputWidget, excludesList, this.contextViewService, {
			...
			placeholder: nls.localize('placeholder.excludes', "ex: **/node_modules/**, **/dist/** (glob patterns)"),
			showPlaceholderOnFocus: true,
		...
		// NeoCode: Always expand query details by default
	...
	private updateIndentStyles(theme: IFileIconTheme): void {
		...
		if (this.fileNameResultsContainer) {
			this.fileNameResultsContainer.classList.toggle('hide-arrows', this.isTreeLayoutViewVisible && theme.hidesExplorerArrows);
		}
	}
	...
	private createSearchWidget(container: HTMLElement): void {
		...
		if (this.searchWidget.searchInputFocusTracker) {
			this._register(this.searchWidget.searchInputFocusTracker.onDidFocus(() => {
				if (!this.queryDetails.classList.contains('more')) {
					this.toggleQueryDetails(true, true, true);
				}
			}));
		}
	...
	private createSearchResultsView(container: HTMLElement): void {
		// NeoCode: File Name Search Results Container
		this.fileNameResultsContainer = dom.append(container, $('.file-name-results.results.show-file-icons.file-icon-themable-tree'));
		this.fileNameResultsContainer.style.display = 'none'; // Hidden by default until results are found
		this.fileNameResultsContainer.style.maxHeight = '250px';
		this.fileNameResultsContainer.style.overflowY = 'auto';
		this.fileNameResultsContainer.style.flexShrink = '0'; // NeoCode: ensure it doesn't get squashed
		this.fileNameResultsContainer.style.borderBottom = '1px solid var(--vscode-panel-border)';
		this.fileNameResultsContainer.style.backgroundColor = 'var(--vscode-sideBar-background)';

		// NeoCode: Text Matches (Content) Header
		this.textMatchesHeader = dom.append(container, $('.text-matches-header', { style: 'padding: 6px 22px; font-weight: 600; font-size: 11px; text-transform: uppercase; opacity: 0.8; border-bottom: 1px solid var(--vscode-panel-border); background-color: var(--vscode-sideBar-background); display: none;' }));
		this.textMatchesHeader.textContent = "Fazendo busca com textos de dentro de arquivos";
	...
	private reLayout(): void {
		...
		let fileNameResultsHeight = 0;
		if (this.fileNameResultsContainer && this.fileNameResultsContainer.style.display !== 'none') {
			fileNameResultsHeight = dom.getTotalHeight(this.fileNameResultsContainer);
		}
		let textHeaderHeight = 0;
		if (this.textMatchesHeader && this.textMatchesHeader.style.display !== 'none') {
			textHeaderHeight = dom.getTotalHeight(this.textMatchesHeader);
		}
		this.tree.layout(this.size.height - widgetHeight - messagesHeight - fileNameResultsHeight - textHeaderHeight, this.size.width - 28);
	}
	...
	clearSearchResults(clearInput = true): void {
		this.clearFileNameResults();
		if (this.textMatchesHeader) {
			this.textMatchesHeader.style.display = 'none';
		}
	...
	toggleFileNameSearch(): void {
		const currentState = this.isFileNameSearchEnabled.get() ?? true;
		const newState = !currentState;
		console.log(`[NeoCode] toggleFileNameSearch: current=${currentState}, new=${newState}`);
		this.isFileNameSearchEnabled.set(newState);
		if (!newState) {
			this.clearFileNameResults();
		} else {
			this.triggerQueryChange({ shouldKeepAIResults: true });
	...
	private doSearch(query: ITextQuery, excludePatternText: string, includePatternText: string, triggeredOnType: boolean, shouldKeepAIResults: boolean, shouldUpdateAISearch: boolean): Thenable<void> {
		...
		// NeoCode: Concurrent file name search
		const filePattern = this.searchWidget.searchInput?.getValue() || '';
		console.log(`[NeoCode] doSearch: filePattern="${filePattern}", enabled=${this.isFileNameSearchEnabled.get()}`);
		if (filePattern.length >= 2 && this.isFileNameSearchEnabled.get() !== false) {
			...
			console.log(`[NeoCode] doSearch: Executing file search...`);
			this.searchService.fileSearch(fileQuery).then(res => {
				console.log(`[NeoCode] doSearch: fileSearch returned ${res.results.length} results.`);
				this.renderFileNameResults(res.results);
			}).catch((err) => {
				console.error(`[NeoCode] doSearch error`, err);
				this.clearFileNameResults();
			});
		} else {
			console.log(`[NeoCode] doSearch: Skipped file search`);
			this.clearFileNameResults();
		}

		// NeoCode: Show text matches header if search pattern exists
		if (filePattern && this.textMatchesHeader) {
			this.textMatchesHeader.style.display = 'block';
		} else if (this.textMatchesHeader) {
			this.textMatchesHeader.style.display = 'none';
		}
	...
	private renderFileNameResults(results: { resource: URI }[]): void {
		console.log(`[NeoCode] renderFileNameResults: Called with ${results.length} items. Container is ${!!this.fileNameResultsContainer}`);
		if (!this.fileNameResultsContainer) { return; }
		dom.clearNode(this.fileNameResultsContainer);

		if (results.length === 0) {
			console.log(`[NeoCode] renderFileNameResults: Empty results, clearing.`);
			this.clearFileNameResults();
			return;
		}

		const wasHidden = this.fileNameResultsContainer.style.display === 'none';
		this.fileNameResultsContainer.style.display = 'block';

		const headerContainer = dom.append(this.fileNameResultsContainer, $('.file-name-results-header', { style: 'padding: 6px 22px; font-weight: 600; font-size: 11px; text-transform: uppercase; opacity: 0.8; display: flex; align-items: center;' }));

		const title = dom.append(headerContainer, $('span'));
		title.textContent = `Files (${results.length})`;

		dom.append(headerContainer, $('span' + ThemeIcon.asCSSSelector(Codicon.info), {
			style: 'margin-left: 6px; cursor: help; opacity: 0.6;',
			title: 'Buscando relações com nomes de arquivos'
		}));

		const list = dom.append(this.fileNameResultsContainer, $('.file-name-results-list'));

		for (const result of results) {
			const item = dom.append(list, $('.file-name-result-item', { tabindex: 0, style: 'display: flex; align-items: center; padding: 2px 22px; cursor: pointer;' }));
			item.addEventListener('mouseenter', () => item.style.backgroundColor = 'var(--vscode-list-hoverBackground)');
			item.addEventListener('mouseleave', () => item.style.backgroundColor = '');

			const labelContainer = dom.append(item, $('.label-container', { style: 'display: flex; width: 100%; overflow: hidden;' }));
			const labelWidget = this.treeLabels.create(labelContainer, { supportHighlights: true });

			labelWidget.setFile(result.resource, { matches: [] });

			item.addEventListener('click', () => {
				this.editorService.openEditor({ resource: result.resource });
			});
	...
	private clearFileNameResults() {
		if (this.fileNameResultsContainer) {
			const wasVisible = this.fileNameResultsContainer.style.display !== 'none';
			this.fileNameResultsContainer.style.display = 'none';
			dom.clearNode(this.fileNameResultsContainer);
			if (wasVisible) {
				this.reLayout();
			}
