...
import { searchClearIcon, searchCollapseAllIcon, searchExpandAllIcon, searchRefreshIcon, searchShowAsList, searchShowAsTree, searchStopIcon, searchFileNameIcon } from './searchIcons.js';
...
// NeoCode
registerAction2(class EnableFileNameSearchAction extends Action2 {
	constructor() {
		super({
			id: Constants.SearchCommandIds.ToggleFileNameSearchActionId,
			title: nls.localize2('EnableFileNameSearchAction.label', "Ativar busca por nomes de arquivos..."),
			icon: searchFileNameIcon, // Files icon
			category,
			f1: true,
			menu: [{
				id: MenuId.ViewTitle,
				group: 'navigation',
				order: 3,
				when: ContextKeyExpr.and(ContextKeyExpr.equals('view', VIEW_ID), Constants.SearchContext.FileNameSearchEnabledKey.toNegated()),
			}]
		});
	}
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		if (searchView) {
			searchView.toggleFileNameSearch();
		}
	}
});

registerAction2(class DisableFileNameSearchAction extends Action2 {
	constructor() {
		super({
			id: 'search.action.disableFileNameSearch', // Internal ID for the disable variant
			title: nls.localize2('DisableFileNameSearchAction.label', "Desativar busca por nomes de arquivos..."),
			icon: Codicon.close, // Let's use close or something distinct
			category,
			f1: false,
			menu: [{
				id: MenuId.ViewTitle,
				group: 'navigation',
				order: 3,
				when: ContextKeyExpr.and(ContextKeyExpr.equals('view', VIEW_ID), Constants.SearchContext.FileNameSearchEnabledKey),
			}]
		});
	}
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
