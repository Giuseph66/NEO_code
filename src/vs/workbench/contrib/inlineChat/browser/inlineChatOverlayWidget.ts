...
export interface IInlineChatOverlayLayoutOptions {
	minWidth: number;
	maxWidth: number;
	maxLines: number;
}
...
export class InlineChatInputWidget extends Disposable {
	...
	private readonly _layoutOptions = observableValue<IInlineChatOverlayLayoutOptions>(this, { minWidth: 220, maxWidth: 600, maxLines: 3 });
	...
	constructor(
		...
		this._layoutData = derived(r => {
			...
			const layoutOptions = this._layoutOptions.read(r);
			const minWidth = layoutOptions.minWidth;
			...
			const clampedHeight = Math.min(contentHeight.read(r), (layoutOptions.maxLines * lineHeight));
	 ...
	 */
	show(lineNumber: number, left: number, anchorAbove: boolean, placeholder: string, layoutOptions?: IInlineChatOverlayLayoutOptions): void {
		this._showStore.clear();

		// Clear input state
		this._input.updateOptions({ wordWrap: 'off', placeholder });
		this._input.getModel().setValue('');
		this._layoutOptions.set(layoutOptions ?? { minWidth: 220, maxWidth: 600, maxLines: 3 }, undefined);
