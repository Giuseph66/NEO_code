...
export interface IInlineChatSelectionMenuOptions {
...
export class InlineChatAffordance extends Disposable {
	...
	readonly #menuData = observableValue<{ rect: DOMRect; above: boolean; lineNumber: number; placeholder: string; menuOptions?: IInlineChatSelectionMenuOptions } | undefined>(this, undefined);
	...
	constructor(
		...
		this._store.add(autorun(r => {
			...
			const layoutOptions = data.menuOptions?.size === 'expanded'
				? { minWidth: 380, maxWidth: 860, maxLines: 8 }
				: undefined;

			// Show the overlay widget
			this.#inputWidget.show(data.lineNumber, left, data.above, data.placeholder, layoutOptions);
		}));
	...
	async showMenuAtSelection(placeholder: string, menuOptions?: IInlineChatSelectionMenuOptions): Promise<void> {
