...
import { createDefaultNeocodeSwarmConfig, INeocodeSwarmAgentConfig, INeocodeSwarmConfig, NEO_SWARM_OPEN_SETTINGS_COMMAND_ID, NEO_SWARM_STORAGE_KEY } from '../../../neocodeSwarm/common/neocodeSwarmTypes.js';
...
interface ISwarmAgentRuntimeState {
	id: string;
	name: string;
	role: string;
...
interface ISwarmEventEntry {
	text: string;
}

interface ISwarmRuntimeState {
	runId: number;
	queryPreview: string;
	status: 'running' | 'complete' | 'error';
	agents: ISwarmAgentRuntimeState[];
	events: ISwarmEventEntry[];
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private swarmActivityContainer!: HTMLElement;
	...
	constructor(
		...
		this.loadSwarmConfig();

		this._register(this.storageService.onDidChangeValue(StorageScope.PROFILE, NEO_SWARM_STORAGE_KEY, this._store)(() => {
			this.loadSwarmConfig();
	...
	private loadSwarmConfig(): void {
		try {
			const raw = this.storageService.get(NEO_SWARM_STORAGE_KEY, StorageScope.PROFILE);
			if (!raw) {
				this.swarmConfig = createDefaultNeocodeSwarmConfig();
				return;
			}
			const parsed = JSON.parse(raw) as Partial<INeocodeSwarmConfig>;
			...
			this.swarmConfig = {
				...
				orchestrator: { ...fallback.orchestrator, ...(parsed.orchestrator ?? {}) },
				advanced: { ...fallback.advanced, ...(parsed.advanced ?? {}) },
	...
	private startSwarmRun(query: string): void {
		if (!this.swarmConfig.swarmEnabled) {
			this.swarmRuntime = undefined;
			this.renderSwarmActivityCard();
			return;
		}

		const queryPreview = query.trim().slice(0, 120);
		const runtimeAgents = this.selectAgentsForSwarmRun(queryPreview).map((agent, index) => ({
			id: agent.id,
			name: agent.name,
			role: this.formatSwarmRole(agent.role),
			status: index === 0 ? 'running' : 'queued'
		} as ISwarmAgentRuntimeState));

		const orchestratorProvider = this.swarmConfig.providers.find(provider => provider.id === this.swarmConfig.orchestrator.providerId);
		this.swarmRunCounter += 1;
		this.swarmRuntime = {
			runId: this.swarmRunCounter,
			queryPreview: queryPreview || localize('chat.swarm.emptyPrompt', "Solicitacao sem texto"),
			status: 'running',
			agents: runtimeAgents,
			events: [
				{ text: localize('chat.swarm.event.received', "Orquestrador recebeu a solicitacao.") },
				{ text: localize('chat.swarm.event.routing', "Roteando tarefas para agentes do enxame.") }
			],
			orchestratorName: orchestratorProvider?.name ?? localize('chat.swarm.orchestratorDefault', "Orquestrador")
		};

		this.swarmSimulationDisposables.clear();
		for (let i = 0; i < runtimeAgents.length; i += 1) {
			const startDelay = 450 * (i + 1);
			const finishDelay = startDelay + 450;
			this.swarmSimulationDisposables.add(disposableTimeout(() => {
				if (!this.swarmRuntime || this.swarmRuntime.runId !== this.swarmRunCounter || this.swarmRuntime.status !== 'running') {
					return;
				}
				for (const agent of this.swarmRuntime.agents) {
					if (agent.status === 'running') {
						agent.status = 'done';
					}
				}
				const current = this.swarmRuntime.agents[i];
				if (current && current.status === 'queued') {
					current.status = 'running';
					this.swarmRuntime.events.push({ text: localize('chat.swarm.event.agentRunning', "{0} executando subtarefa.", current.name) });
				}
				this.renderSwarmActivityCard();
			}, startDelay));
			this.swarmSimulationDisposables.add(disposableTimeout(() => {
				...
				if (current && current.status === 'running') {
					current.status = 'done';
					this.swarmRuntime.events.push({ text: localize('chat.swarm.event.agentDone', "{0} finalizou a etapa.", current.name) });
				}
				this.renderSwarmActivityCard();
			}, finishDelay));
		}
	...
	private getSwarmRoutingValidationMessage(): string | undefined {
		if (!this.swarmConfig.swarmEnabled) {
			return localize('chat.swarm.configureOrchestrator.enableSwarm', "Configure seu orquestrador para iniciar o chat.");
		}

		const orchestratorProviderId = this.swarmConfig.orchestrator.providerId?.trim();
		if (!orchestratorProviderId) {
			return localize('chat.swarm.configureOrchestrator.missingProvider', "Configure seu orquestrador para iniciar o chat.");
		}

		const provider = this.swarmConfig.providers.find(candidate => candidate.id === orchestratorProviderId);
		if (!provider) {
			return localize('chat.swarm.configureOrchestrator.invalidProvider', "Orquestrador invalido. Abra as configuracoes do enxame.");
		}

		if (!provider.enabled) {
			return localize('chat.swarm.configureOrchestrator.disabledProvider', "Ative o provedor do orquestrador para iniciar o chat.");
		}

		return undefined;
	}

	private showSwarmRoutingBlocked(message: string, query: string): void {
		const queryPreview = query.trim().slice(0, 120) || localize('chat.swarm.emptyPrompt', "Solicitacao sem texto");
		const orchestratorProvider = this.swarmConfig.providers.find(provider => provider.id === this.swarmConfig.orchestrator.providerId);
		this.swarmRunCounter += 1;
		this.swarmRuntime = {
			runId: this.swarmRunCounter,
	...
	private updateSwarmRunProgress(message: string): void {
		if (!this.swarmRuntime || this.swarmRuntime.status !== 'running') {
			return;
		}
		this.swarmRuntime.events.push({ text: message });
		this.renderSwarmActivityCard();
	}

	private completeSwarmRun(message: string): void {
		if (!this.swarmRuntime) {
			return;
		}
		this.swarmRuntime.status = 'complete';
		for (const agent of this.swarmRuntime.agents) {
	...
	private failSwarmRun(message: string): void {
		if (!this.swarmRuntime) {
			return;
		}
		this.swarmRuntime.status = 'error';
		const runningAgent = this.swarmRuntime.agents.find(agent => agent.status === 'running') ?? this.swarmRuntime.agents[0];
		if (runningAgent) {
	...
	private renderSwarmActivityCard(): void {
		if (!this.swarmActivityContainer) {
			return;
		}

		const shouldShow = !isInlineChat(this) && !isQuickChat(this);
		dom.setVisibility(shouldShow, this.swarmActivityContainer);
		if (!shouldShow) {
			return;
		}

		this.swarmUiDisposables.clear();
		...
		const statusText = !this.swarmConfig.swarmEnabled
			...
			: !this.swarmRuntime
				? localize('chat.swarm.status.idle', "Aguardando solicitacao")
				: this.swarmRuntime.status === 'running'
					? localize('chat.swarm.status.running', "Em execucao")
					: this.swarmRuntime.status === 'complete'
						? localize('chat.swarm.status.complete', "Concluido")
						: localize('chat.swarm.status.error', "Erro");
	...
	private renderSwarmAgentsSummary(parent: HTMLElement): void {
		if (!this.swarmRuntime?.agents.length) {
			return;
		}

		const strip = dom.append(parent, $('.chat-swarm-agents-strip'));
		for (const agent of this.swarmRuntime.agents) {
			const chip = dom.append(strip, $(`span.chat-swarm-agent-chip.chat-swarm-agent-${agent.status}`));
			dom.append(chip, $('span.chat-swarm-agent-name', undefined, agent.name));
			dom.append(chip, $('span.chat-swarm-agent-role', undefined, agent.role));
		}
	}

	private renderSwarmEventsTimeline(parent: HTMLElement): void {
		if (!this.swarmRuntime?.events.length) {
			return;
		}

		const timeline = dom.append(parent, $('.chat-swarm-timeline'));
		for (const eventEntry of this.swarmRuntime.events.slice(-8)) {
			dom.append(timeline, $('div.chat-swarm-event', undefined, eventEntry.text));
		}
	}

	private selectAgentsForSwarmRun(query: string): INeocodeSwarmAgentConfig[] {
		const configured = this.swarmConfig.agents.filter(agent => agent.active);
		if (!configured.length) {
			return [];
		}

		const lower = query.toLowerCase();
		const scored = configured.map(agent => {
			let score = 0;
			if (agent.role === 'planner') {
				score += 2;
			}
			if (agent.role === 'coder' && /(code|build|implementar|criar|fix)/.test(lower)) {
				score += 2;
			}
			if (agent.role === 'reviewer' && /(review|revis|test|qa)/.test(lower)) {
				score += 2;
			}
			if (agent.role === 'researcher' && /(research|pesquis|investigar|analisar)/.test(lower)) {
				score += 2;
			}
			if (agent.role === 'debugger' && /(bug|erro|debug|falha)/.test(lower)) {
				score += 2;
			}
			return { agent, score };
		});
		...
		const selected = scored.slice(0, Math.max(1, Math.min(this.swarmConfig.advanced.concurrency, configured.length))).map(item => item.agent);
		return selected;
	}

	private formatSwarmRole(role: INeocodeSwarmAgentConfig['role']): string {
		switch (role) {
			case 'planner':
				return localize('chat.swarm.role.planner', "Planejador");
			case 'coder':
				return localize('chat.swarm.role.coder', "Codificador");
			case 'reviewer':
				return localize('chat.swarm.role.reviewer', "Revisor");
			case 'researcher':
				return localize('chat.swarm.role.researcher', "Pesquisador");
			case 'debugger':
				return localize('chat.swarm.role.debugger', "Depurador");
			case 'custom':
			default:
				return localize('chat.swarm.role.custom', "Personalizado");
		}
	}
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.installed) {
					...
					additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with Neo Agents, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3}).", providers.default.name, providers.default.name, product.defaultChatAgent.termsStatementUrl, product.defaultChatAgent.privacyStatementUrl), { isTrusted: true });
				} else {
	...
	setModel(model: IChatModel | undefined): void {
		...
		this.viewModelDisposables.add(model.onDidChange((e) => {
			...
			if (e.kind === 'addRequest') {
				...
				this.updateSwarmRunProgress(localize('chat.swarm.requestSent', "Solicitacao enviada para o orquestrador."));
			}
			...
			if (e.kind === 'removeRequest') {
				...
				this.failSwarmRun(localize('chat.swarm.requestRemoved', "Solicitacao removida antes da conclusao."));
			}
			...
			if (e.kind === 'completedRequest') {
				...
				if (wasCancelled) {
					...
					this.inputPart.clearTodoListWidget(this.viewModel?.sessionResource, true);
					this.failSwarmRun(localize('chat.swarm.cancelled', "Execucao cancelada."));
				}
				// Only show if response wasn't canceled
				this.renderChatSuggestNextWidget();
				if (!wasCancelled) {
					this.completeSwarmRun(localize('chat.swarm.complete', "Resposta consolidada pelo orquestrador."));
				}
	...
	async rerunLastRequest(): Promise<void> {
		...
		const swarmValidationError = this.getSwarmRoutingValidationMessage();
		if (swarmValidationError) {
			this.showSwarmRoutingBlocked(swarmValidationError, localize('chat.swarm.rerunPrompt', "Reenviar ultima solicitacao"));
			return;
		}
	...
	private async _acceptInput(query: { query: string } | undefined, options: IChatAcceptInputOptions = {}): Promise<IChatResponseModel | undefined> {
		...
		const swarmValidationError = this.getSwarmRoutingValidationMessage();
		if (swarmValidationError) {
			this.showSwarmRoutingBlocked(swarmValidationError, requestInputs.input);
			return;
		}

		this.startSwarmRun(requestInputs.input);

		let result: Awaited<ReturnType<IChatService['sendRequest']>>;
		try {
			...
			this.failSwarmRun(localize('chat.swarm.sendError', "Falha ao enviar solicitacao do enxame."));
			throw error;
		}

		if (ChatSendResult.isRejected(result)) {
			this.failSwarmRun(localize('chat.swarm.rejected', "Solicitacao rejeitada."));
			return;
		}
		...
		if (!ChatSendResult.isSent(sent)) {
			this.failSwarmRun(localize('chat.swarm.notSent', "Nao foi possivel iniciar o processamento."));
			return;
		}
	...
	layout(height: number, width: number): void {
		...
		const swarmCardHeight = this.getSwarmCardHeight();
		const lastElementVisible = this.listWidget.isScrolledToBottom;
		const lastItem = this.listWidget.lastItem;

		const contentHeight = Math.max(0, height - inputHeight - chatSuggestNextWidgetHeight - swarmCardHeight);
	...
	private getSwarmCardHeight(): number {
		if (!this.swarmActivityContainer || this.swarmActivityContainer.style.display === 'none') {
			return 0;
		}
		return this.swarmActivityContainer.offsetHeight;
	}
