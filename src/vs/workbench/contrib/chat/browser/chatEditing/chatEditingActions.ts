...
export function getEditingSessionContext(accessor: ServicesAccessor, args: any[]): EditingSessionActionContext | undefined {
	...
	if (chatWidget?.viewModel) {
		editingSession = chatEditingService.getEditingSession(chatWidget.viewModel.model.sessionResource);
	}

	// Fallback for inline/quick-edit sessions: resolve by active editor resource.
	if (!editingSession) {
		const activeResource = EditorResourceAccessor.getCanonicalUri(editorService.activeEditor, { supportSideBySide: SideBySideEditor.PRIMARY });
		if (activeResource) {
			editingSession = chatEditingService.editingSessionsObs.get().find(session => session.getEntry(activeResource));
		}
	}

	// Last resort: if there is exactly one undecided editing session, use it.
	if (!editingSession) {
		const undecidedSessions = chatEditingService.editingSessionsObs.get().filter(session =>
			session.entries.get().some(entry => entry.state.get() === ModifiedFileEntryState.Modified)
		);
		if (undecidedSessions.length === 1) {
			editingSession = undecidedSessions[0];
		}
	}
