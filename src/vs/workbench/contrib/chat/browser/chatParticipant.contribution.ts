...
const chatViewIcon = registerIcon('chat-view-icon', Codicon.hubot, localize('chatViewIcon', 'View icon of the Neo Agents view.'));
...
const chatViewContainer: ViewContainer = Registry.as<IViewContainersRegistry>(ViewExtensions.ViewContainersRegistry).registerViewContainer({
	...
	title: localize2('chat.viewContainer.label', "Neo Agents"),
	icon: chatViewIcon,
...
const chatViewDescriptor: IViewDescriptor = {
	...
	name: localize2('chat.viewContainer.label', "Neo Agents"),
	canToggleVisibility: false,
	canMoveView: true,
	openCommandActionDescriptor: {
		id: ChatViewContainerId,
		title: chatViewContainer.title,
		mnemonicTitle: localize({ key: 'miToggleChat', comment: ['&& denotes a mnemonic'] }, "&&Neo Agents"),
		keybindings: {
...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Neo Agents failed to load because the installed chat extension is not compatible with this version of {0}. Please ensure that the chat extension is up to date.", this.productService.nameLong);
